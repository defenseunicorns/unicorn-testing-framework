...
func setupEC2(t *testing.T, options EC2Options) {
	randomInstanceName := fmt.Sprintf("%s-%s", options.InstanceName, random.UniqueId())
	keyPair, err := aws.CreateAndImportEC2KeyPairE(t, options.Region, randomInstanceName)
	require.NoError(t, err)
	terraformOptions := terraform.WithDefaultRetryableErrors(t, &terraform.Options{
		TerraformDir: options.TestFolder,
	...
	err = waitForInstanceReady(t, options, 5*time.Second, 15) //nolint:gomnd
	require.NoError(t, err)
	ssh.ScpFileToE(t)
}

func teardownEC2(t *testing.T, options EC2Options) {
	keyPair := teststructure.LoadEc2KeyPair(t, options.TestFolder)
	terraformOptions := teststructure.LoadTerraformOptions(t, options.TestFolder)
	terraform.Destroy(t, terraformOptions)
	aws.DeleteEC2KeyPair(t, keyPair)
}

func runSSHCommandWithOptionalSudo(t *testing.T, options EC2Options, command string, asSudo bool) (string, error) {
	precommand := "bash -c"
	if asSudo {
...
func waitForInstanceReady(t *testing.T, options EC2Options, timeBetweenRetries time.Duration, maxRetries int) error {
	t.Helper()
	_, err := retry.DoWithRetryE(t, "Wait for the instance to be ready", maxRetries, timeBetweenRetries, func() (string, error) {
		_, err := runSSHCommandWithOptionalSudo(t, options, "whoami", true)
		if err != nil {
...
// func copyFileToInstanceE(t *testing.T, options EC2Options, remotePath string, contents string) error {
// 	terraformOptions := teststructure.LoadTerraformOptions(t, options.TestFolder)
// 	keyPair := teststructure.LoadEc2KeyPair(t, options.TestFolder)
// 	host := ssh.Host{
// 		Hostname:    terraform.Output(t, terraformOptions, "public_instance_ip"),
